...
export const list: controller = (_req, res, next) => {
    Article.find({}).populate("topic", "author").exec((err: Error, articles: unknown[]) => {
        if (err) {
            next(err);
        }
        res.json(articles);
...
export const remove: controller = async (req, res, next) => {
    try {
        ...
        next(err);
        res.send("Deleting article failed. Maybe check that you got the ID right.\n");
    }
};
...
export const getArticle: controller = async (req, res, next) => {
    try {
        const article = await Article.findById(req.params.id).populate("topic", "author");
        res.send(article);
    } catch (err) {
        next(err);
        res.send(
