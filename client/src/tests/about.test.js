...
it("the titles of the page", () => {
    ...
    ReactDOM.render(<About />, div);
    const allHeaders = [];
    [...div.querySelectorAll("h1")].forEach((element) => allHeaders.push(element.textContent))
...
it("renders the introductory paragraph", () => {
    const div = document.createElement("div");
    ReactDOM.render(<About />, div);
    expect(div.querySelector(".descriptionText").textContent).toEqual("The Vanderbilt Undergraduate Microbiome Society aims to educate the local community about the microbiome and the field’s latest developments, to promote social and microbial diversity, and to help facilitate the distribution of fresh produce to underprivileged persons in Davidson County.");
});

it("renders the contact paragraph", () => {
    const div = document.createElement("div");
    ReactDOM.render(<About />, div);
    expect(div.querySelectorAll(".descriptionText")[1].textContent).toEqual("Contact us through our AnchorLink!");
});
